...
| J04 — Sessions + second Master | ✅ PASS technique — en attente revue humaine | `4ad8956` | `199e1c5` | `tests/e2e/validation/J04-sessions/` | Transport WS LAN (serveur WebSocket embarqué générique, patch minimal Java-WebSocket 1.6.0, clients `new WebSocket`, `http://localhost`-origine + `ws://`, SPA mono-document, action native `status` interdite). Campaign 4 `tests/e2e/j04-campaign.sh clean` : **9/9 PASS technique** sur 2 Android physiques (B/C) — création `sessionId`/PIN/annonce DNS-SD, découverte LAN, join 2 Masters convergence identique, renommage propagé + TXT, PIN erroné rejeté sans toucher la session, fermeture propagée + LAN vidé, re-jonction fermée refusée, restart même session conservée, purge stale. APK SHA-256 `2e91bf1bc5…ac9f107`. Tests unitaires fusion 7/7. Bugs réels corrigés : purge cache `session-store.remove()`, dedup annonce DNS-SD, noop sync-echo, throttle reSync, mapping NACK closed/unknown. Scénarios 3 Masters différés par décision utilisateur (A bloqué `INSTALL_FAILED_USER_RESTRICTED`, HyperOS) — non affaiblis. Détails : `VALIDATION.md` du dossier |
