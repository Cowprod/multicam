...
| J03 — Découverte LAN / mDNS | 🔎 Validé (attente revue humaine) | `5df8321` | `fb5f606` | `tests/e2e/validation/J03-decouverte-mdns/` | Découverte mDNS/DNS-SD réelle validée contre un pair publié du Mac (`dns-sd -P`) + client tiers (`dns-sd -L/-B`) : table de pairs keyée deviceId sans doublon, found/resolve/lost primaire, re-convergence 3 cycles + relances app, re-annonces renommage/skills, `enabled` omis quand vide, `/health` minimal corrélé, écran 01 reflétant l'état réel ; **réservation Android↔Android LEVÉE par campaign 2** (`mdns3-test.sh`, rc=0 sur 3 devices physiques, APK `6b65ecc1…56b2`) : échange en place (S6/S7/S8), bascule Wi-Fi multi-pairs + cache NSD/TTL (S10/S10b), stop/restart ×3 + même nom ×2 devices (S11/S12). Défauts d'outillage corrigés dans le harness (baseline Phase 0, précondition S6, oracle `MDNS_RESOLVE`). **Implementation testée exactly = `5df8321`** (code natif NSD : re-annonce en deux temps, retry + refresh découverte — exact du binaire validé, verrouillé byte-for-byte après run, aucune modif de comportement ; `42fcadb` = baseline J03 d'origine). Défaut `settings.js` (accolade en trop) trouvé/corrigé pendant la campaign 1 |
